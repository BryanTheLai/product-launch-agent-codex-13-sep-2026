...
## Launch Room Creative Agent Operating Rules

The Launch Room Creative Agent is an autonomous D2C Creative Director in Slack (`apps/channel`, `CHANNEL_CODE=launch-2027`):

- **Skills Directory**: Located in `.agents/skills/`. Includes 17 Launch Room creative skills and 8 Higgsfield gen-ai skills.
- **Product Identity Invariance**: Never allow generative product drift. Enforce the `ProductIdentitySpec` across all packshots, catalog angles, lifestyle scenes, macro textures, and poster variations.
- **5-Block Prompt Composition**: Product images generated via OpenAI Flare (`gpt-image-2.5-flare-2026-09-08`) must compose all 5 blocks: `[PRODUCT SPECIFICATION]`, `[CREATIVE TREATMENT]`, `[SHOT PLAN]`, `[NEGATIVE CONSTRAINTS]`, and `[CLAIMS POLICY]`.
- **Live Market Intelligence**: Exa searches must return real, inspectable source URLs. Never cache Exa results. Label all parameters as `observed`, `inferred`, `assumption`, or `user-supplied`.
- **Unit Economics Formula**: Stripe card processing fee is strictly `2.9% + $0.30` per domestic transaction. Use Shopify 2026 category benchmark of `2.85% CVR`.
- **Presentation Decks**: 16:9 widescreen PPTX (`pptxgenjs`) and matching PDF (`pdf-lib`) must be editable and adhere to the warm brown/cream skincare aesthetic.
- **In-Thread Iteration**: Aesthetic edits (e.g. "Make Poster B more retro") regenerate only Poster B and update the deck without re-calling Exa. Commercial edits (e.g. supplier quote) update economics and slides without touching images.
- **Automated Verification**: Run `npm run verify` (108 tests) and `npm run demo` (3-phase automated smoke test).
