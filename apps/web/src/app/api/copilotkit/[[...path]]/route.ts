...
// The Mission Room gives the agent a workplace when the operator has configured
// one. The page still owns the explicit consequential-action checkpoint; normal
// research, drafting, task creation, and reversible workplace work can continue.
const runtime = new CopilotRuntime({
  agents: () => ({
    default: makeAgent(randomUUID(), {
      workplace: Boolean(process.env.AMBIGUOUS_API_KEY?.trim()),
    }),
  }),
});
