...
const marker = (missionId: string) =>
  `agents-everywhere:${findMission(missionId).id}`;
...
export class FollowupService {
  ...
  async propose(session: string, input: unknown): Promise<Proposal> {
    ...
    const mission = findMission(draft.incidentId);
    ...
    const proposal = {
      ...
      title: draft.title,
      description: `${draft.details}\n\nMission Room: ${mission.id} — ${mission.title}\n${marker(mission.id)}\nfollowup:${actionKey}`,
