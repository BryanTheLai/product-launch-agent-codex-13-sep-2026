...
const setup =
  "Set AMBIGUOUS_API_KEY in root .env and restart the web app. Explore sample missions now; saving and retrieval require a real Ambiguous workspace.";
      ...
      if (error instanceof z.ZodError || error instanceof SyntaxError)
        return await withCleanup(
          reply(
            {
              error:
                "Invalid request or provider data. Check the mission, title, details, and record ID.",
            },
            400,
          ),
        );
