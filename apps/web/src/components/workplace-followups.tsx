    ...
    try {
      ...
      setError(
        error instanceof Error
          ? error.message
          : "Unable to refresh workplace artifacts from Ambiguous.",
      );
    }
  }
  ...
  return (
    <section className="ck-followups" aria-labelledby="followup-title">
      <header className="ck-followups-header">
        <div>
          <h2 id="followup-title">Work artifacts</h2>
          <p className="ck-local-note">
            The agent can prepare workplace actions freely. Save only after
            you review the exact fields; refresh reads the provider again.
          </p>
        </div>
      ...
      {status?.status === "unconfigured" ? (
        <div className="ck-setup-note">
          <strong>Connect a workspace to save artifacts</strong>
          <p>{status.message}</p>
          <p>
            You can still explore the Mission Room. No browser-only record
            will be created as a stand-in.
          </p>
        </div>
      ...
      {tasks.length ? (
        ...
        <p className="ck-empty">No saved artifacts for {incidentId}.</p>
      ) : null}
      ...
      <form onSubmit={submit} className="ck-task-form ck-task-form--stacked">
        <label className="ck-sr-only" htmlFor="task-title">
          New workplace action for {incidentId}
        </label>
        <input
          id="task-title"
          value={title}
          ...
          maxLength={200}
          placeholder="Write an artifact title..."
          required
        />
      ...
      {proposal && (
        <section className="ck-approval" aria-label="Approve Ambiguous task">
          <h3>Review this workplace action</h3>
          <p>
          ...
          <p>This becomes a real Ambiguous write. Review the exact fields above.</p>
