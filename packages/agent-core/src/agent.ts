...
import { workplaceMcpClients } from "./capabilities/workplace";
...
export type AgentFactoryOptions = {
  ...
  workplace?: boolean;
  /** Override the default Mission Room prompt for a surface-specific starter. */
  prompt?: string;
};
...
export function makeAgent(threadId: string, options: AgentFactoryOptions = {}) {
  const agent = new BuiltInAgent({
    ...
    // agent is never handed tools that would 401. Ambiguous's user-managed
    // MCP client preserves the schema wrapper while adapting unsupported regex.
    // the same way — note HTTP transport takes `options` (with a wrapped
    mcpClients: options.workplace === false ? [] : [...workplaceMcpClients()],
  });
